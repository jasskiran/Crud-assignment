...
func GenerateJWT(userId int) (string, error) {
	...
	tokenString, err := token.SignedString([]byte(ViperConfigVariable("jwt_secret.jwtsecret")))
	viper.Set("token.jwt_token", tokenString)
...
func AuthRequired(handlerFunc func(w http.ResponseWriter, r *http.Request)) func(w http.ResponseWriter, r *http.Request) {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		token, err := jwt.ParseWithClaims(tokenPart, &tk, func(token *jwt.Token) (interface{}, error) {
			return []byte(ViperConfigVariable("jwt_secret.jwtsecret")), nil
		})
